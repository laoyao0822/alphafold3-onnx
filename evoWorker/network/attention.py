...
import evoWorker.config as config
from evoWorker.network.dot_product_attention import dot_product_attention,dot_product_attention_sdpa
...
class GridSelfAttention(nn.Module):
    ...
    def _attention(self,num_res,mask):
        # print("attention device:",self.q_projection.weight.device)
        seq_len=num_res

        pair = torch.zeros((seq_len, seq_len,self.c_pair), dtype=torch.bfloat16,device=self.q_projection.weight.device).contiguous()
        print("start to receive",pair.shape)
        ...
        weighted_avg2=dot_product_attention_sdpa(q2, k2, v2,attn_mask=mask,bias=bias2)
        ...
        # dist.reduce(out_proj2, dst=0, op=dist.ReduceOp.SUM)
        dist.send(tensor=out_proj2, dst=0)
        print("success send")
        # return out_proj2

    def forward(self,num_res,mask):
        if self.rank==1 and num_res>config._GridAttention_TP_Min_TOKEN and config._GridAttention_TP :
            self.chunk_weight()
            self._attention(num_res,mask)
