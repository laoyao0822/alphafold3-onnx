...
class ConfidenceHead(nn.Module):
    ...
    def __init__(self, c_single: int = 384, c_pair: int = 128, c_target_feat: int = 447, n_pairformer_layers=4):
        ...
        self.distogram_feat_project = nn.Linear(
            self.drag_num_bins, self.c_pair, bias=False)
    ...
    def dgram_from_positions(self,positions: torch.Tensor):
        ...
        lower_breaks = torch.linspace(
            self.drag_min_bin, self.drag_max_bin, self.drag_num_bins, device=positions.device)
    ...
    def forward(
        ...
        pred_distance_error = (
            torch.sum(distance_probs * self.bin_centers, dim=-1)
        )
    ...
    def _get_tmscore_adjusted_pae(self,
        ...
        num_interface_tokens = num_interface_tokens
